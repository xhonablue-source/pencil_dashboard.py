# Vocabulary Section
st.markdown("---")
st.markdown("### 📚 Mathematical Vocabulary")

# Interactive vocabulary with definitions and examples
vocab_tabs = st.tabs(["📐 Measurement", "📊 Area & Geometry", "🧮 Mathematical Thinking"])

with vocab_tabs[0]:
    st.markdown("#### Measurement Terms")
    vocab_col1, vocab_col2 = st.columns(2)
    
    with vocab_col1:
        st.markdown("""
        **Estimation** 📏  
        *Making an educated guess about a quantity before measuring*  
        Example: "I estimate this pencil is about 7 inches long"
        
        **Measurement** 🔍  
        *Finding the exact size using standard tools*  
        Example: Using a ruler to find the pencil is exactly 7.5 inches
        
        **Unit** 📋  
        *A standard amount used for measuring*  
        Example: Inches, centimeters, tape strips
        """)
    
    with vocab_col2:
        st.markdown("""
        **Precision** 🎯  
        *How exact and detailed a measurement is*  
        Example: 7.25 inches is more precise than 7 inches
        
        **Conversion** ↔️  
        *Changing from one unit to another*  
        Example: 1 inch = 2.54 centimeters
        
        **Standard vs. Non-standard** ⚖️  
        *Standard: inches, cm | Non-standard: tape strips, paper clips*
        """)

with vocab_tabs[1]:
    st.markdown("#### Area & Geometry Terms")
    vocab_col3, vocab_col4 = st.columns(2)
    
    with vocab_col3:
        st.markdown("""
        **Area** ▢  
        *Amount of space inside a 2D shape or covering a surface*  
        Measured in square units (square inches, square cm)
        
        **Surface Area** 📦  
        *Total area of all surfaces of a 3D object*  
        Example: All the area you'd cover wrapping a pencil
        
        **Cross-section** ✂️  
        *What you see when you cut straight through an object*  
        Example: Cutting a pencil shows a circular cross-section
        """)
    ...
    with vocab_col4:
        ...
        *Distance around the edge of a circle*  
        Example: How far around the pencil's cross-section
        
        **Cylinder** 🥫  
        *A 3D shape with circular ends and curved sides*  
        Example: A pencil is approximately a cylinder
        
        **Dimensions** 📏  
        *Measurements that describe size*  
        Example: Length, width, height, thickness
        """)

with vocab_tabs[2]:
    st.markdown("#### Mathematical Thinking Terms")
    vocab_col5, vocab_col6 = st.columns(2)
    
    with vocab_col5:
        st.markdown("""
        **Formula** 🧮  
        *A mathematical rule written with symbols*  
        Example: Area = Length × Width × Number of strips
        
        **Pattern** 🔄  
        *A repeated or predictable arrangement*  
        Example: Noticing your estimates are usually too low
        
        **Strategy** 🎯  
        *A planned approach to solving problems*  
        Example: Breaking complex shapes into simpler parts
        """)
    
    with vocab_col6:
        st.markdown("""
        **Reasoning** 🧠  
        *Logical thinking to explain why something is true*  
        Example: "The area doubles because both dimensions doubled"
        
        **Mathematical Model** 🏗️  
        *Using math to represent real-world situations*  
        Example: Using our formula to predict tape needed
        
        **Variable** 📝  
        *A symbol that represents a changing number*  
        Example: In our formula, length and width are variables
        """)

# Area Formula Highlight
st.markdown("---")
